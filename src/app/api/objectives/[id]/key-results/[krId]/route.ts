...
export async function PATCH(req: NextRequest, { params }: { params: { id: string; krId: string } }) {
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string; krId: string } }) {
  const user = await getCurrentUser();
  if (!user) return unauthorized();
