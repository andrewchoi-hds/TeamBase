...
async function main() {
  ...
  const template = await prisma.reviewTemplate.create({
    data: {
      ...
      guideline: `평가 작성 가이드라인

1. 구체적인 행동 사례를 중심으로 평가해주세요.
2. 평가 대상자의 성장 가능성을 고려하여 건설적인 피드백을 작성해주세요.
3. 각 항목별로 1(기대 미달)~5(탁월) 점수를 부여하고, 가능하면 코멘트를 추가해주세요.
4. "보통" 수준은 3점이며, 차별적 성과가 있을 때 4~5점을 부여합니다.
5. 평가의 일관성을 유지하기 위해 동일한 기준으로 모든 대상자를 평가해주세요.`,
      isDefault: true,
      categories: {
        create: [
          {
            ...
            criteria: {
              create: [
                { name: "전문 지식", description: "담당 업무 분야의 전문 지식 수준", order: 0, options: { rubric: { "1": "기본 지식 부족, 빈번한 실수", "2": "기초 수준, 가이드 필요", "3": "업무 수행에 충분한 수준", "4": "깊은 전문성, 동료 멘토링 가능", "5": "분야 전문가, 조직 내 기술 리더" } } },
                { name: "문제 해결력", description: "업무 과제를 분석하고 해결하는 능력", order: 1, options: { rubric: { "1": "문제 인식이 부족, 외부 도움 필수", "2": "단순 문제만 해결 가능", "3": "일반적 문제를 독립적으로 해결", "4": "복잡한 문제도 체계적으로 해결", "5": "혁신적 접근으로 근본 원인 해결" } } },
